...
// header portion. This is relevant to PacketHeader.Push method only.
...
func (pk *PacketBuffer) Size() int {
	...
	return pk.Data.Size() +
		len(pk.headers[transportHeader].buf) +
		len(pk.headers[networkHeader].buf) +
		len(pk.headers[linkHeader].buf)
}
...
func (pk *PacketBuffer) Views() []buffer.View {
	...
	useHeader := canUseHeader(&pk.headers[transportHeader]) &&
		canUseHeader(&pk.headers[networkHeader]) &&
		canUseHeader(&pk.headers[linkHeader])
	...
	if useHeader {
		...
		vs = make([]buffer.View, 0, int(numHeaderType)+len(dataViews))
...
func (pk *PacketBuffer) consume(typ headerType, size int) (buffer.View, bool) {
	h := &pk.headers[typ]
	if h.buf != nil {
...
// Push pushes h of size bytes in the front of its residing packet, and returns
// the backing storage. Callers may only call one of Push or Consume in the
...
// to h, and returns the backing storage. Callers may only call one of Push or
// Consume in the lifetime of the underlying packet.
...
func PayloadSince(h PacketHeader) buffer.View {
	var v buffer.View
	for i := h.typ; i < numHeaderType; i++ {
		v = append(v, h.pk.headers[i].buf...)
	}
