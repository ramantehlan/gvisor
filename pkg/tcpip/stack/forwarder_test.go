...
func (e *fwdTestLinkEndpoint) WriteRawPacket(vv buffer.VectorisedView) *tcpip.Error {
	p := fwdTestPacketInfo{
		Pkt: NewPacketBuffer(&NewPacketBufferOptions{Data: vv}),
	}
...
func TestForwardingWithStaticResolver(t *testing.T) {
	...
	buf[dstAddrOffset] = 3
	ep1.InjectInbound(fwdTestNetNumber, NewPacketBuffer(&NewPacketBufferOptions{
...
func TestForwardingWithFakeResolverManyPackets(t *testing.T) {
	...
	for i := 0; i < maxPendingPacketsPerResolution; i++ {
		...
		seqNumBuf := b[fwdTestNetHeaderLen:][:2] // The sequence number is a uint16 (2 bytes).
