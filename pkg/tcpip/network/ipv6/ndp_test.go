...
func TestNeighorSolicitationWithSourceLinkLayerOption(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e.InjectInbound(ProtocolNumber, stack.NewPacketBuffer(&stack.NewPacketBufferOptions{
...
func TestNeighorSolicitationResponse(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e.InjectLinkAddr(ProtocolNumber, test.nsSrcLinkAddr, stack.NewPacketBuffer(&stack.NewPacketBufferOptions{
				Data: hdr.View().ToVectorisedView(),
			}))
...
func TestRouterAdvertValidation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			e.InjectInbound(header.IPv6ProtocolNumber, stack.NewPacketBuffer(&stack.NewPacketBufferOptions{
				Data: hdr.View().ToVectorisedView(),
			}))
