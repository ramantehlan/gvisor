...
func makeRandPkt(hdrLength int, extraLength int, viewSizes []int) *stack.PacketBuffer {
	...
	rand.Read(pkt.TransportHeader().Push(hdrLength))
	return pkt
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, pkt := range tc.packets {
				ep.InjectLinkAddr(header.IPv4ProtocolNumber, remoteLinkAddr, stack.NewPacketBuffer(&stack.NewPacketBufferOptions{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, frag := range test.fragments {
				...
				e.InjectInbound(header.IPv4ProtocolNumber, stack.NewPacketBuffer(&stack.NewPacketBufferOptions{
					Data: vv,
				}))
			}
