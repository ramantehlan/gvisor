...
// Package refs defines a template that can be used by reference counted
// objects.
package refs
...
// Refs keeps a reference count using atomic operations and calls the destructor
...
func (r *Refs) finalize() {
	...
	if n := r.ReadRefs(); n != 0 {
		msg := fmt.Sprintf("%sAtomicRefCount %p owned by %T garbage collected with ref count of %d (want 0)", note, r, owner, n)
		log.Warningf(msg)
	}
}
...
// IncRef increments this object's reference count.
//
//go:nosplit
func (r *Refs) IncRef() {
...
// TryIncRef attempts to increment the reference count. It returns false if the
// object has already been destroyed. If true is returned then a valid reference
// is now held on the object.
//
...
// DecRefWithDestructor decrements the object's reference count and calls the
// given destructor if no references remain.
//
...
//
//go:nosplit
func (r *Refs) DecRefWithDestructor(destroy func()) {
